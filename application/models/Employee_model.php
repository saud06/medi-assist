...
class Employee_model extends CI_Model
{
	...
	*/
	...
	/* 
		return employee details to promote 
	*/
	public function getEmployeeToPromote($id){
		//$b_cat_id = "SELECT SUBSTRING(category_id, 1, 1) FROM client";
		$data = $this->db->select('*')
		                 ->from('employee')
		                 ->where('id', $id)
	...
	/* 
		return all employee details to dispaly list 
	*/
	public function getFilteredEmployee($status){
		if(empty($status)){
	...
	*/
	public function addModel($employee_data){
		$sql = "insert into employee (employee_id,nid_number,dob,first_name,last_name,nick_name,fname,mname,religion,marital_status,passport_no,gender,blood_group,nationality,cphone,cemail,pre_address,per_address,emp_photo,emp_resume,join_desg,join_date,start_time,end_time,starting_salary,wk_holiday,emp_status,user_id,datetime) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		if($this->db->query($sql,$employee_data)){
	...
	*/
	public function editModel($employee_data,$id){
		...
		//if($this->db->query($sql,$data)){
		$this->db->where('id', $id);
		if($this->db->update('employee', $employee_data)){
			return TRUE;
		}
		else{
			return FALSE;
	...
	*/
	public function addLeave($data){
		...
		if($result){
			$final_data = array(
				"leave_status" => 'y',
				"leave_remarks" => $data['leave_remarks']
			);
			...
			if($this->db->update('attendance', $final_data)){
				return TRUE;
			}
			else{
				return FALSE;
			...
			for($i=0; $i<sizeof($dates); $i++){
				$sql = "INSERT INTO `attendance`(`ac_no`, `name`, `leave_status`, `leave_remarks`, `auto_assign`, `date`, `time_table`, `on_duty`, `off_duty`, `clock_in`, `clock_out`, `normal`, `real_time`, `late`, `early`, `absent`, `ot_time`, `work_time`, `exception`, `must_c_in`, `must_c_out`, `department`, `n_days`, `week_end`, `holiday`, `att_time`, `n_days_ot`, `week_end_ot`, `holiday_ot`) VALUES ('".$emp_id."','".$data['name']."','".$data['leave_status']."','".$data['leave_remarks']."','','".$dates[$i]."','','','','','','','','','','','','','','','','','','','','','','','')";

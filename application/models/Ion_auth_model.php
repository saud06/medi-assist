...
class Ion_auth_model extends CI_Model
{
	...
	*/
	...
	/* 
		delete user record in databse 
	*/
	public function deleteModel($id){
		$sql = "delete from users where id = ?";
		if($this->db->query($sql,array($id))){
			$sql2 = "delete from users_groups where user_id = ?";
			if($this->db->query($sql2,array($id))){
				return TRUE;
			}
			else{
				return FALSE;
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', token, email, username, id, password, user_status, first_name, last_name, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
			...
			if ($password === TRUE)
			{
				...
				if($this->session->userdata('user_id') != 1){
				    $query = $this->db->select('*')
                                  ->where(array('user_id'=>$this->session->userdata('user_id'), 'group_id'=>'1'))
                                  ->get($this->tables['users_groups']);
				    
				    if ($query->num_rows() > 0){
				        $this->db->update($this->tables['users'], array('token' => NULL), array('id' => $user->id));
    					...
    					if ($user->token == NULL){
    						// insert token for initial user login
    						$token = uniqid(rand(), true);
    						$this->db->update($this->tables['users'], array('token' => $token), array('id' => $user->id));
    						...
    						if(isset($_COOKIE['user_token']) && !empty($_COOKIE['user_token'])){
    							if($_COOKIE['user_token'] == $user->token) {
    								$this->trigger_events(array('post_login', 'post_login_successful'));
    								$this->set_message('login_successful');
    								...
    								$this->trigger_events('post_login_unsuccessful');
    								$this->set_error('login_unauthorized');
        ...
        if($recheck!==0)
        {
            ...
            if($last_login+$recheck < time())
            {
                $query = $this->db->select('id')
                    ->where(array($this->identity_column=>$this->session->userdata('identity'),'user_status'=>'1'))
                    ->limit(1)
                    ->order_by('id', 'desc')
                    ->get($this->tables['users']);
                if ($query->num_rows() === 1)
	...
	public function filteredUsers($filt_category_id, $status, $groups = NULL)
	{
		$this->trigger_events('users');
		...
		if($filt_category_id == 0 && $status == 2){
			$this->response = $this->db->get($this->tables['users']);
		}

		// category NULL, status available
		elseif($filt_category_id == 0 && $status < 2){
			$this->response = $this->db->where('user_status', $status)
									   ->get($this->tables['users']);
		}

		// category N/A, status NULL
		elseif($filt_category_id == -1 && $status == 2){
			$this->response = $this->db->where('category_id', NULL)
									   ->get($this->tables['users']);
		}

		// category N/A, status available
		elseif($filt_category_id == -1 && $status < 2){
			$this->response = $this->db->where('category_id', NULL)
		...
		// category available, status available
			...
			$this->response = $this->db->where('category_id', $filt_category_id)
									   ->where('user_status', $status)
									   ->get($this->tables['users']);
		}
	...
	* @author Phil Sturgeon
	**/
	public function reset_token($data, $id){
		$sql = "update users set token=?, user_id=?, datetime=? where id = ?";
		if($this->db->query($sql,array($data['token'],$data['user_id'],$data['datetime'],$id))){
	...
	**/
	...
	public function allReports(){
	    ...
	    /*  Products  */
		$data['todayProduct'] = $this->db->select('count(*) as item')->where('datetime',date("Y-m-d"))->get('products')->result();
		$data['weekProduct'] = $this->db->select('count(*) as item')->where('datetime BETWEEN "'.$from.'" AND "'.$to.'"')->get('products')->result();
		$data['monthProduct'] = $this->db->select('count(*) as item')->like('datetime',date('Y-m'))->get('products')->result();
		$data['yearProduct'] = $this->db->select('count(*) as item')->like('datetime',date('Y'))->get('products')->result();
		$data['allProduct'] = $this->db->select('count(*) as item')->get('products')->result();

		/*  Remittance Total Order, Total Earning */
		$data['weekTotOrdErn'] = $this->db->select('COUNT(*) AS tot_order, SUM(grand_total) AS tot_earning')
							  			 ->from('remittance_history')
								  		 ->where('email_date BETWEEN "'.$from.'" AND "'.$to.'"')
								  		 ->get()->result();

		$data['monthTotOrdErn'] = $this->db->select('COUNT(*) AS tot_order, SUM(grand_total) AS tot_earning')
					  					  ->from('remittance_history')
								  		  ->like('email_date',date('Y-m'))
								  		  ->get()->result();
		
		$data['yearTotOrdErn'] = $this->db->select('COUNT(*) AS tot_order, SUM(grand_total) AS tot_earning')
								  		 ->from('remittance_history')
								  		 ->like('email_date',date('Y'))
								  		 ->get()->result();
		
		$data['allTotOrdErn'] = $this->db->select('COUNT(*) AS tot_order, SUM(grand_total) AS tot_earning')
								  		->from('remittance_history')
								  		->get()->result();

		/*  Remittance Total Customer */
		$data['weekTotCust'] = $this->db->select('COUNT(*) AS tot_customer')
								  		 ->from('remittance_history')
								  		 ->where('email_date BETWEEN "'.$from.'" AND "'.$to.'"')
								  		 ->group_by('customer_id')
								  		 ->get()->result();

		$data['monthTotCust'] = $this->db->select('COUNT(*) AS tot_customer')
								  		 ->from('remittance_history')
								  		 ->like('email_date',date('Y-m'))
								  		 ->group_by('customer_id')
								  		 ->get()->result();
		
		$data['yearTotCust'] = $this->db->select('COUNT(*) AS tot_customer')
								  		->from('remittance_history')
								  		->like('email_date',date('Y'))
								  		->group_by('customer_id')
								  		->get()->result();
		
		$data['allTotCust'] = $this->db->select('COUNT(*) AS tot_customer')
							  		   ->from('remittance_history')
								  	   ->group_by('customer_id')
								  	   ->get()->result();

		/*  Remittance Target */
		$data['remittanceTarget'] = $this->db->select('*')
								  		 ->from('remittance_target')
								  		 ->get()->result();

		/*  Purchase */
		$data['todayPurchase'] = $this->db->select('sum(pi.quantity) as item, SUM( IF( p.currency_id = 1, (														pi.gross_total), 0) ) AS value_in_usd, SUM( IF( p.currency_id = 2, (										pi.gross_total), 0) ) AS value_in_bdt')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->where('p.date', date('Y-m-d'))
								  ->get()->result();
		
		$data['weekPurchase'] = $this->db->select('sum(pi.quantity) as item, SUM( IF( p.currency_id = 1, (														pi.gross_total), 0) ) AS value_in_usd, SUM( IF( p.currency_id = 2, (										pi.gross_total), 0) ) AS value_in_bdt')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->where('p.date BETWEEN "'.$from.'" AND "'.$to.'"')
								  ->get()->result();

		$data['monthPurchase'] = $this->db->select('sum(pi.quantity) as item, SUM( IF( p.currency_id = 1, (														pi.gross_total), 0) ) AS value_in_usd, SUM( IF( p.currency_id = 2, (										pi.gross_total), 0) ) AS value_in_bdt')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->like('p.date',date('Y-m'))
								  ->get()->result();
		
		$data['yearPurchase'] = $this->db->select('sum(pi.quantity) as item, SUM( IF( p.currency_id = 1, (														pi.gross_total), 0) ) AS value_in_usd, SUM( IF( p.currency_id = 2, (										pi.gross_total), 0) ) AS value_in_bdt')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->like('p.date',date('Y'))
								  ->get()->result();
		
		$data['allPurchase'] = $this->db->select('sum(pi.quantity) as item, SUM( IF( p.currency_id = 1, (														pi.gross_total), 0) ) AS value_in_usd, SUM( IF( p.currency_id = 2, (										pi.gross_total), 0) ) AS value_in_bdt')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->get()->result();

		/* Sales */
		$data['todaySales'] = $this->db->select('sum(si.quantity) as item, SUM( IF( s.currency_id = 1, (														si.gross_total), 0) ) AS value_in_usd, SUM( IF( s.currency_id = 2, (										si.gross_total), 0) ) AS value_in_bdt')
								  ->from('sales s')
		...
		$data['weekSales'] = $this->db->select('sum(si.quantity) as item, SUM( IF( s.currency_id = 1, (														si.gross_total), 0) ) AS value_in_usd, SUM( IF( s.currency_id = 2, (										si.gross_total), 0) ) AS value_in_bdt')
								  ->from('sales s')
								  ->join('sales_items si','s.sales_id = si.sales_id')
								  ->where('s.date BETWEEN "'.$from.'" AND "'.$to.'"')
								  ->get()->result();

		$data['monthSales'] = $this->db->select('sum(si.quantity) as item, SUM( IF( s.currency_id = 1, (														si.gross_total), 0) ) AS value_in_usd, SUM( IF( s.currency_id = 2, (										si.gross_total), 0) ) AS value_in_bdt')
								  ->from('sales s')
								  ->join('sales_items si','s.sales_id = si.sales_id')
								  ->like('s.date',date('Y-m'))
								  ->get()->result();
		
		$data['yearSales'] = $this->db->select('sum(si.quantity) as item, SUM( IF( s.currency_id = 1, (														si.gross_total), 0) ) AS value_in_usd, SUM( IF( s.currency_id = 2, (										si.gross_total), 0) ) AS value_in_bdt')
								  ->from('sales s')
								  ->join('sales_items si','s.sales_id = si.sales_id')
								  ->like('s.date',date('Y'))
								  ->get()->result();
		
		$data['allSales'] = $this->db->select('sum(si.quantity) as item, SUM( IF( s.currency_id = 1, (														si.gross_total), 0) ) AS value_in_usd, SUM( IF( s.currency_id = 2, (										si.gross_total), 0) ) AS value_in_bdt')
								  ->from('sales s')
								  ->join('sales_items si','s.sales_id = si.sales_id')
								  ->get()->result();
		...
		$notice_result = $this->db->order_by('notice_id', 'DESC')->limit('3')->get('notice')->result();
		$data['notice'] = array_reverse($notice_result);
		$data['notice2'] = $this->db->order_by('notice_id', 'DESC')->limit('1')->get('notice')->result();
	...
	public function updateTotAmount($data){
		$this->db->update('remittance_target', $data);
	...
	public function saveNote($note){
		...
		$this->db->insert('sticky_note', $data);
	...
	public function saveNotes($notes){
		...
		for($i=0; $i<sizeof($notes); $i++){
			if(!empty($notes[$i])){
				...
				$this->db->select('*')
					 ->where('description', $notes[$i])
                     ->where('user_id', $this->session->userdata('user_id'));
              	$notes_array = $this->db->get('sticky_note')->result_array();

				if(sizeof($notes_array) == 0){
					$this->db->insert('sticky_note', $data);
				}
			}
	...
	public function removeNote($id){
		$sql = "DELETE FROM sticky_note WHERE id = ?";

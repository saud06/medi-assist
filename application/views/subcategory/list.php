    ...
    <section class="content-header">
      <h5>
         <ol class="breadcrumb">
          ...
          <li class="active"><!-- Subcategory -->Subcategory List</li>
        </ol>
      </h5>
    </section>
    <!-- Main content -->
      ...
      <div class="col-md-12">
        <div class="box bo">
            <div class="box-header with-border">
              ...
              <a title="Add New Subcategory" class="btn bg-gray" style="margin: 10px" href="<?php echo base_url('subcategory/add');?>"><i class="fa fa-plus" aria-hidden="true"></i></a>
            </div>
            <!-- /.box-header -->
            <div class="box-body outer-scroll">
              <div class="inner-scroll">
                <div class="row">
                  <div class="col-sm-12">
                    <strong>Subcategory List Filter:</strong> &emsp;
                    ...
                    <button title="Filter Subcategory List" type="button" id="btn_ajax" class="btn bg-gray"><i class="fa fa-search" aria-hidden="true"></i></button>&emsp;

                    <div class="btn-group">
                      <button title="Print Subcategory List" type="button" class="btn bg-gray gropdown-toggle" data-toggle="dropdown">
                      ...
                      <ul class="dropdown-menu">
                        ...
                        <li id="pdf">
                          <a href="<?php echo base_url('subcategory/list_pdf');?>" target="_blank">PDF (On Letter Head)</a>
                        </li>
                      </ul>
                    </div>
                  </div>
                            ...
                            <?php if($this->session->userdata('type') == 'admin'){ ?>
                              <td>
                                <div class="dropdown">
                                  ...
                                  <ul class="dropdown-menu">
                                    <li>
                                      <a href="<?php echo base_url('subcategory/edit/'); ?><?php echo $id; ?>"><i class="fa fa-edit"></i>Edit</a>
                                    </li>
                                    <li>
...
<script>
  $('#btn_ajax').click(function(){
    ...
    $.ajax({
      url: "<?php echo base_url('subcategory/filter_subcategory') ?>/",
      type: "POST",
      dataType: "JSON",
      data : {
        status: status
      ...
      success: function (response) {
        ...
        $.each(response.data, function (i, item) {
          trHTML += '<tr>';
          trHTML += '<td>'+(i+1)+'</td>';
          trHTML += '<td>' + item.sub_category_code + '</td>';
          trHTML += '<td>' + item.sub_category_name + '</td>';
          trHTML += '<td>' + item.category_name + '</td>';
          trHTML += '<td>' + item.sub_category_desc + '</td>';
          var type = '<?php echo $this->session->userdata("type") ?>';
          if(type == 'admin'){
            trHTML += '<td>';
              trHTML += '<div class="dropdown"><button title="Action" type="button" class="btn btn-sm bg-gray gropdown-toggle" data-toggle="dropdown"><i class="fa fa-cog" aria-hidden="true"></i> &nbsp;<span class="fa fa-angle-double-down"></span></button><ul class="dropdown-menu"><li><a href="<?php echo base_url('subcategory/edit/'); ?>' + item.sub_category_id + '"><i class="fa fa-edit"></i>Edit</a></li><li><a href="javascript:delete_id(' + item.sub_category_id + ')"><i class="fa fa-trash-o"></i>Delete</a></li></ul></div>';
            trHTML += '</td>';
          }
          trHTML += '</tr>';
        });
    ...
    $("#pdf").html('<a href="<?php echo base_url('subcategory/filterSubcategoryPDF/');?>' + status + '" target="_blank">PDF (On Letter Heads)</a>');
  });

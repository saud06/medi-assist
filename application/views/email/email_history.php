    ...
    <section class="content-header">
      <h5>
        <ol class="breadcrumb">
          ...
          <li class="active">Work History</li>
        </ol>
      </h5>    
    </section>
    <!-- Main content -->
        ...
        <div class="row">
          ...
          <div class="col-md-10">
            <div class="box">
            	<div class="box-header with-border">
	                <h3 class="box-title">
	                  <?php if($this->uri->segment('3') == 'inquiry_query'){ echo 'Inquiry Mail'; } elseif($this->uri->segment('3') == 'quotation_query'){ echo 'Quotation'; } elseif($this->uri->segment('3') == 'indent_query'){ echo 'Proforma Invoice / Indent'; } else{ echo 'Forwarding / O.C'; } ?> History
	                </h3>
	            </div>

	            <div class="box-body outer-scroll">
	                <div class="inner-scroll">
		                <form class="form-group" name="actionForm1" id="actionForm1" action="<?php echo base_url('email/delete_mul'); ?>" method="post"/>
		                  <div class="box-body no-padding">
		                    <div class="table-responsive mailbox-messages">
		                      <div class="row" style="width: 100%">
		                        <div class="col-md-3" style="padding-right: 0">
		                          <div class="box box-solid">
		                            <input type="text" name="email_id" id="email_id" class="form-control" placeholder="Filter with Reference No.">
		                          </div>
		                        </div>

		                        <div class="col-md-3" style="padding-right: 0">
		                          <div class="box box-solid">
		                            <input type="text" name="email_address" id="email_address" class="form-control" placeholder="Filter with Client Name">
		                          </div>
		                        </div>

		                        <div class="col-md-1">
		                          <button title="Filter" type="button" id="btn_ajax" class="btn bg-gray"><i class="fa fa-search" aria-hidden="true"></i></button>
		                        </div>
		                      </div>
		                      ...
		                      <table class="table table-hover table-striped">
		                        <tbody id="records2">
		                          <?php 
		                              foreach ($email_saved[$this->uri->segment('3')] as $row){
		                                $body = explode(" ", strip_tags($row->body));
		                                $pieces = implode(" ", array_splice($body, 0, 5));

                                    if(substr($row->ref_no, 5, 2) == 'SD'){
		                                  $sType = 'sd';
		                                  $lType = 'sample_draft';
		                                }
		                                else if(substr($row->ref_no, 5, 3) == 'QTN'){
		                                  $sType = 'qt';
		                                  $lType = 'quotation';
		                                }
		                                else if(substr($row->ref_no, 5, 3) == 'IND'){
		                                  $sType = 'it';
		                                  $lType = 'indent';
		                                }
		                                else if(substr($row->ref_no, 5, 3) == 'INQ'){
		                                  $sType = 'iq';
		                                  $lType = 'inquiry';
		                                }
		                          ?>
		                                <tr>
		                                  <td style="width: 5%"><input name="selected_id1[]" value="<?php echo $sType . $row->id; ?>" type="checkbox"></td>
		                                  <td width="20%"><b>[</b><a id="email" href="../read_email/<?php echo $lType . '/' . $row->id; ?>"><?= $row->ref_no ?></a><b>]</b></td>
		                                  <td width="20%" class="mailbox-name"><?php echo $row->customer_name; ?></td>
		                                  <td width="35%" class="mailbox-subject"><?php if($row->subject) echo $row->subject . ' - '; echo $pieces . '...'; ?>
		                                  </td>
		                                  <td width="15%" class="mailbox-date"><?php echo $row->datetime; ?></td>
		                                </tr>
		                          <?php
		                              }
		                          ?>
		                  ...
		                  <div class="box-footer no-padding">
		                    <div class="mailbox-controls">
		                      ...
		                      <div class="btn-group">
		                        <button type="submit" class="btn btn-default btn-sm" <?php if(empty($row->id)){?> disabled <?php }?>><i class="fa fa-trash-o"></i></button>
		                      </div>
		                    </div>
		                  </div>
		                </form>
      ...
      <div class="modal fade" id="modal_form3" role="dialog">
        <div class="modal-dialog" style="width: 80%">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <table id="index3" class="table table-bordered table-striped product_table1">
                <thead>
                  <th>No.</th>
                  <th>Date</th>
                  <th>Reference No.</th>
...
<script>
  ...
  $(document).ready(function(){
    $(document).on('keyup keypress', 'form#actionForm1 input[type="text"]', function(e) {
      if(e.keyCode == 13){
        e.preventDefault();
          ...
          $.ajax({
            ...
            success: function (response) {
              ...
              for(i=0; i<email_saved.sample_draft_query.length; i++) {
                ...
                if(email_saved.sample_draft_query[i].ref_no.substr(5, 2) == 'SD'){
                  var sType = 'sd';
                ...
                ref_no_arr.push(email_saved.sample_draft_query[i].ref_no);
                mail_no_arr.push(email_saved.sample_draft_query[i].mail_no);
                mail_too_arr.push(email_saved.sample_draft_query[i].too);
                mail_client_arr.push(email_saved.sample_draft_query[i].customer_name);
              ...
              for(j=0; j<email_saved.quotation_query.length; j++) {
                ...
                if(email_saved.quotation_query[j].ref_no.substr(5, 3) == 'QTN'){
                  var sType = 'qt';
                  var lType = 'quotation';
                }
                ...
                mail_sType_arr.push(sType);
                mail_lType_arr.push(lType);
                ref_no_arr.push(email_saved.quotation_query[i].ref_no);
                mail_no_arr.push(email_saved.quotation_query[j].mail_no);
                mail_too_arr.push(email_saved.quotation_query[j].too);
                mail_client_arr.push(email_saved.quotation_query[j].customer_name);
              ...
              for(k=0; k<email_saved.indent_query.length; k++) {
                ...
                if(email_saved.indent_query[k].ref_no.substr(5, 3) == 'IND'){
                  var sType = 'it';
                  var lType = 'indent';
                }
                ...
                mail_sType_arr.push(sType);
                mail_lType_arr.push(lType);
                ref_no_arr.push(email_saved.indent_query[k].ref_no);
                mail_no_arr.push(email_saved.indent_query[k].mail_no);
                mail_too_arr.push(email_saved.indent_query[k].too);
                mail_client_arr.push(email_saved.indent_query[k].customer_name);
              ...
              if(email_saved.indent_query[k].ref_no.substr(5, 3) == 'INQ'){
                ...
                mail_sType_arr.push(sType);
                mail_lType_arr.push(lType);
                ref_no_arr.push(email_saved.inquiry_query[l].ref_no);
                mail_no_arr.push(email_saved.inquiry_query[l].mail_no);
                mail_too_arr.push(email_saved.inquiry_query[l].too);
                mail_client_arr.push(email_saved.inquiry_query[l].customer_name);
              ...
              for(m=0; m<counter; m++){
                ...
                trHTML += '<td width="20%"><b>[</b><a target="_blank" id="email" href="email/read_email/' + mail_lType_arr[m] + '/' + mail_id_arr[m] + '">' + ref_no_arr[m] + '</a><b>]</b></td>';
                trHTML += '<td width="20%" class="mailbox-name">' + mail_client_arr[m] + '</td>';
                trHTML += '<td width="35%" class="mailbox-subject"><b>' + mail_subject_arr[m] + ' - ' + mail_pieces_arr[m] + '...</b></td>';
                trHTML += '<td width="15%" class="mailbox-date">' + mail_datetime_arr[m] + '</td>';
                trHTML += '</tr>';
              }
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            for(l=0; l<email_saved.inquiry_query.length; l++) {
              var body = $(email_saved.inquiry_query[l].body).text();
              var exp_body = body.split(" ");
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $.each(response.data, function (i, item) {
          ...
          trHTML += '<td width="10%"><b>[</b><a target="_blank" id="email" href="email/read_email/' + ele + '/' + item.id + '">' + item.mail_no + '</a><b>]</b></td>';
          trHTML += '<td width="20%" class="mailbox-name">' + item.too + '</td>';

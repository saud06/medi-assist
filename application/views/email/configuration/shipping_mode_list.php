...
<script type="text/javascript">
  function delete_id(id)
  {
   ...
   {
    window.location.href='<?php  echo base_url('email/shipping_mode_delete/'); ?>'+id;
  }
}
</script>
  ...
  <section class="content-header">
    <h5>
     <ol class="breadcrumb">
      ...
      <li class="active"><!-- shelf --> <!-- <?php echo $this->lang->line('shelf_label'); ?> -->Shipping Mode</li>
    </ol>
  </h5> 
</section>
<!-- Main content -->
    ...
    <div class="col-md-12">
      <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title">Shipping Mode List</h3>
          <a title="Add Shipping Mode" class="btn bg-gray" style="margin: 10px" href="<?php echo base_url('email/shipping_mode_add');?>"><i class="fa fa-plus"></i></a>
        </div>
        <!-- /.box-header -->
        <div class="box-body outer-scroll">
          <div class="inner-scroll">
            <div class="row">
              <div class="col-sm-12">
                <strong>Shipping Mode List Filter:</strong> &emsp;
                ...
                <button title="Filter Shipping Mode List" type="button" id="btn_ajax" class="btn bg-gray"><i class="fa fa-search"></i></button>&emsp;

                <div class="btn-group">
                  <button title="Print Shipping Mode List" type="button" class="btn bg-gray gropdown-toggle" data-toggle="dropdown">
                  ...
                  <ul class="dropdown-menu">
                    ...
                    <li id="pdf">
                      <a href="<?php echo base_url('email/shipping_mode_list_pdf');?>" target="_blank">PDF (On Letter Head)</a>
                    </li>
                  </ul>
                </div>
              </div>
                  ...
                  <?php if($this->session->userdata('type') == 'admin'){ ?> 
                    <td>
                      <div class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                          <li>
                            <a href="<?php echo base_url('email/shipping_mode_edit/'); ?><?php echo $id; ?>"><i class="fa fa-edit"></i>Edit</a>
                          </li>
                          <li>
...
<script>
  $('#btn_ajax').click(function(){
    ...
    $.ajax({
      url: "<?php echo base_url('email/filter_shipping_mode') ?>/",
      type: "POST",
      dataType: "JSON",
      data : {
        status: status
      ...
      success: function (response) {
        ...
        $.each(response.data, function (i, item) {
          trHTML += '<tr>';
          trHTML += '<td>'+(i+1)+'</td>';
          trHTML += '<td>' + item.shipping_mode_ec_name + '</td>';
          trHTML += '<td>' + item.shipping_mode_ec_details + '</td>';
          var type = '<?php echo $this->session->userdata("type") ?>';
          if(type == 'admin'){
            trHTML += '<td>';
              trHTML += '<div class="dropdown"><button title="Action" type="button" class="btn btn-sm bg-gray gropdown-toggle" data-toggle="dropdown"><i class="fa fa-cog"></i> &nbsp;<span class="fa fa-angle-double-down"></span></button><ul class="dropdown-menu"><li><a href="<?php echo base_url('email/shipping_mode_edit/'); ?>' + item.shipping_mode_ec_id + '"><i class="fa fa-edit"></i>Edit</a></li><li><a href="javascript:delete_id(' + item.shipping_mode_ec_id + ')"><i class="fa fa-trash-o"></i>Delete</a></li></ul></div></td>';
            trHTML += '</td>';
          }
          trHTML += '</tr>';
        });
    ...
    $("#pdf").html('<a href="<?php echo base_url('email/filterShippingModePDF/');?>' + status + '" target="_blank">PDF (On Letter Heads)</a>');
  });

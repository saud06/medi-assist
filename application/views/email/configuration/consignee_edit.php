    ...
    <section class="content-header">
      <h5>
        <ol class="breadcrumb">
          ...
          <li class="active"><!-- Edit consignee_ec_ --> Consignee Edit</li>
        </ol>
      </h5>
    </section>
    <!-- Main content -->
      ...
      <div class="col-md-12">
        <div class="box">
            <div class="box-header with-border">
              <h3 class="box-title">Consignee Edit</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <div class="col-sm-6">
                <form role="form" id="form" method="post" action="<?php echo base_url('email/editConsignee');?>">
                  <?php foreach($data as $row){?>
                    <div class="form-group">
                      <label for="consignee_ec_name">Consignee Name<span class="validation-color">*</span></label>
                      <input type="text" class="form-control" id="consignee_ec_name" name="consignee_ec_name" value="<?php echo $row->consignee_ec_name; ?>">
                      <input type="hidden" name="id" value="<?php echo $row->consignee_ec_id;?>">
                      <span class="validation-color" id="err_consignee_ec_name"><?php echo form_error('consignee_ec_name'); ?></span>
                    </div>
                       <div class="form-group">
                      <label for="consignee_ec_details">Deatils</label>
                      <input type="text" class="form-control" id="consignee_ec_details" name="consignee_ec_details" value="<?php echo $row->consignee_ec_details; ?>" >
                    </div>

                    <div class="form-group">
                      <label for="consignee_ec_address">Address</label>
                      <input type="text" class="form-control" id="consignee_ec_address" name="consignee_ec_address" value="<?php echo $row->consignee_ec_address; ?>" >
                    </div>

                    <div class="form-group">
                      <label for="category_name"><!-- Category Description --> Status </label><br>
                      <?php echo lang('category_status_y_label', 'confirm');?>
                      <input type="radio" name="confirm" value="active" <?php if($row->consignee_ec_status == 'active') {?> checked="checked" <?php }?> class="minimal"/>&nbsp;&nbsp;
                      <?php echo lang('category_status_n_label', 'confirm');?>
                      <input type="radio" name="confirm" value="inactive" <?php if($row->consignee_ec_status != 'active') {?> checked="checked" <?php }?> class="minimal"/>
                    </div>
                  ...
                  <div class="box-footer">
                    ...
                    <span class="btn btn-danger" id="cancel" style="margin-left: 2%" onclick="cancel('email/consignee_list')"><!-- Cancel --><!--  <?php echo $this->lang->line('consignee_ec_label_cancel'); ?> -->Cancel</span>
                  </div>
                </div>
                   <?php } ?>
                </form>

...
<script type="text/javascript">
  function delete_id(id)
  {
   ...
   {
    window.location.href='<?php  echo base_url('email/commission_delete/'); ?>'+id;
  }
}
</script>
    ...
    <div class="col-md-12">
      <div class="box">
        <div class="box-header with-border">
          ...
          <a title="Add New Commission" class="btn bg-gray" style="margin: 10px" href="<?php echo base_url('email/Commission_add');?>"><i class="fa fa-plus"></i></a>
        </div>
        <!-- /.box-header -->
        <div class="box-body outer-scroll">
          <div class="inner-scroll">
            <div class="row">
              <div class="col-sm-12">
                <strong>Commission List Filter:</strong> &emsp;
                ...
                <button title="Filter Commission List" type="button" id="btn_ajax" class="btn bg-gray"><i class="fa fa-search"></i></button>&emsp;

                <div class="btn-group">
                  <button title="Print Commission List" type="button" class="btn bg-gray gropdown-toggle" data-toggle="dropdown">
                  ...
                  <ul class="dropdown-menu">
                    ...
                    <li id="pdf">
                      <a href="<?php echo base_url('email/commission_list_pdf');?>" target="_blank">PDF (On Letter Head)</a>
                    </li>
                  </ul>
                </div>
              </div>
                  ...
                  <?php if($this->session->userdata('type') == 'admin'){ ?>
                    <td>
                      <div class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                          <li>
                            <a href="<?php echo base_url('email/commission_edit/'); ?><?php echo $id; ?>"><i class="fa fa-edit"></i>Edit</a>
                          </li>
                          <li>
...
<script>
  $('#btn_ajax').click(function(){
    ...
    $.ajax({
      url: "<?php echo base_url('email/filter_commission') ?>/",
      type: "POST",
      dataType: "JSON",
      data : {
        status: status
      ...
      success: function (response) {
        ...
        $.each(response.data, function (i, item) {
          trHTML += '<tr>';
          trHTML += '<td>'+(i+1)+'</td>';
          trHTML += '<td>' + item.commission_ec_name + '</td>';
          trHTML += '<td>' + item.commission_ec_value + '</td>';
          trHTML += '<td>' + item.commission_ec_details + '</td>';
          var type = '<?php echo $this->session->userdata("type") ?>';
          if(type == 'admin'){
            trHTML += '<td>';
              trHTML += '<div class="dropdown"><button title="Action" type="button" class="btn btn-sm bg-gray gropdown-toggle" data-toggle="dropdown"><i class="fa fa-cog"></i> &nbsp;<span class="fa fa-angle-double-down"></span></button><ul class="dropdown-menu"><li><a href="<?php echo base_url('email/commission_edit/'); ?>' + item.commission_ec_id + '"><i class="fa fa-edit"></i>Edit</a></li><li><a href="javascript:delete_id(' + item.commission_ec_id + ')"><i class="fa fa-trash-o"></i>Delete</a></li></ul></div></td>';
            trHTML += '</td>';
          }
          trHTML += '</tr>';
        });
    ...
    $("#pdf").html('<a href="<?php echo base_url('email/filterCommissionPDF/');?>' + status + '" target="_blank">PDF (On Letter Heads)</a>');
  });

            ...
            <div class="box-body outer-scroll">
              <div class="inner-scroll">
                <div class="row">
                  ...
                  <div class="col-sm-2">
                    <button title="Filter Receipt List" type="button" id="btn_ajax" class="btn bg-gray"><i class="fa fa-search"></i></button> &emsp;
                    
                    <div class="btn-group">
                      <button title="Print Receipt List" type="button" class="btn bg-gray gropdown-toggle" data-toggle="dropdown">
                      ...
                      <ul class="dropdown-menu">
                        ...
                        <li id="pdf">
                          <a href="<?php echo base_url('purchase/receipt_list_pdf');?>" target="_blank">PDF (On Letter Head)</a>
                        </li>
                      </ul>
                    </div>
                  </div>
                  ...
                  <tbody id="records">
                    <?php 
                      ...
                      <tr>
                        ...
                        <td>
                            <a href="<?php echo base_url('purchase/view/');?><?php echo $id; ?>" title="view"><i class="fa fa-file-text-o bg-gray"></i></a>
                        </td>
                      </tr>
                      <?php
                        }
...
<script>
  $('#btn_ajax').click(function(){
    ...
    $.ajax({
      url: "<?php echo base_url('purchase/filterReceipt') ?>/",
      type: "POST",
      dataType: "JSON",
      data : {
      ...
      success: function (response) {
        ...
        $.each(response.data, function (i, item) {
          trHTML += '<tr>';
          trHTML += '<td>' + (i+1) + '</td>';
          // trHTML += '<td>' + item.receipt_voucher_date + '</td>';
          trHTML += '<td align="center">' + item.invoice_no + '</td>';
          trHTML += '<td align="right">';
            ...
            trHTML += item.receipt_amount;
          trHTML += '</td>';
          var type = '<?php echo $this->session->userdata("type") ?>';
          if(type == 'admin'){
            trHTML += '<td>';
              trHTML += '<a href="<?php echo base_url('purchase/view/');?>' + item.purchase_id + '" title="view"><i class="fa fa-file-text-o bg-gray"></i></a>';
            trHTML += '</td>';
          }
          trHTML += '</tr>';
        });
    ...
    $("#pdf").html('<a href="<?php echo base_url('purchase/filterReceiptPDF/');?>' + new_start_date + '/' + new_end_date + '" target="_blank">PDF (On Letter Heads)</a>');
  });
